...
- Creature attacks that the catalog source lists as items (troll log swings, draugr and skeleton weapons, charred greatswords, brute taunts) and internal cheat items no longer appear among player weapons; they sit under "Creature and Internal". Bombs, pickaxes, tools, keys, eggs, tankards, fishing gear, fist weapons, and the scythe now have their own glyph ids (placeholder tiles until the round-2 art lands) instead of borrowing the sword or greatsword.
