...
### Added
- A categorised item picker for adding inventory items: curated groups on the left, an icon grid with search on the right, and an Advanced tab for raw modded prefabs. Inventory slots now show item icons. Icons are drawn placeholders until the original glyph set lands in `assets/glyphs/items/`, at which point the art is used automatically.
